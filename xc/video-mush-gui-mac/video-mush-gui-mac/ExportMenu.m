...
- (NSString *)genCsv:(NSString *)splitChar
{
    ...
    for (int i = 0; i < rowCount; i++) {
        for (NSTableColumn *col in cols) {
            NSTextField * cell = [self.tableView.delegate tableView:self.tableView viewForTableColumn:col row:i];
            //id cell = [self.tableView.dataSource tableView:self.tableView objectValueForTableColumn:col row:i];
            [data appendFormat:@"\"%@\"%@", [(cell.stringValue != nil ? cell.stringValue : @"") stringByReplacingOccurrencesOfString:@"\"" withString:@"\"\""], splitChar];
        }
        [data appendString:@"\n"];
    }
    return [data copy];
