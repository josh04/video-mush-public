                    ...
                    local float *buffer)
{
    ...
    const int off = mad24(ylid + 1, stride, xlid + 1);
    ...
    for(int i = 0; i < num_pixels; i++, ip = ip + 3)
    {
        const int offset = ip[0];
        //const int offset = 0;
        const int xx = (short)(offset & 0xffffu);
        const int yy = (short)(offset >> 16);
        const int idx = mad24(yy, stride, xx) + off;
        //if (idx >= 0 && idx < 18*18) {
            sum[ip[2]] = sum[ip[2]] + buffer[idx] * ip[1];
        //}
    }
